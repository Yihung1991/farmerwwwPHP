...
<div class="container position-relative">
  ...
  $brand_sql = "SELECT COUNT(1) FROM products_brand_category WHERE nid =0";
  ...
  if ($totalRows > 0) {
    ...
    $brand_sql = sprintf("SELECT * FROM `products_brand_category`  WHERE nid =0 ORDER BY `sid` DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">品牌列表</li>
    </ol>
  </nav>
  <section class="">
    <!-- 頁碼 -->
    ...
    <div class="mt-2">
      ...
      <tbody>
        ...
        <?php foreach ($rows as $key => $r) : ?>
          ...
          <td><?= $r['name'] ?></td>
          <td>
            <a href="product_edit-brand.php?sid=<?= $r['sid'] ?>">
